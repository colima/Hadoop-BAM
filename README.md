...
| |`hadoopbam.bam.max-intervals-for-split-filtering`|`1000`|The maximum number of intervals for which splits will be trimmed or filtered. This is an optimization setting, since if there are a large number of intervals (many thousands) then it is more efficient to use all splits and only perform filtering at the read level.|
